...
private:
	...
	UnisocFrameInfo *findFrameInfo(Request *request)
	{
		...
		ASSERT(!info || info->request == request);
		return info;
	}
...
void UnisocCameraData::statsBufferReady(FrameBuffer *buffer)
{
	...
	UnisocFrameInfo *info = findFrameInfo(request);
	if (!info) {
		/* This can happen when the capture buffer fails. */
		LOG(Unisoc, Debug) << "Dropping stats buffer " << buffer;
...
int PipelineHandlerUnisoc::configure(Camera *camera, CameraConfiguration *c)
{
	...
	ret = data->sensor_->setFormat(&subdevFormat, config->combinedTransform_);
	if (ret < 0)
		return ret;
